...
def _patch_nodriver_connection_instance(connection_instance):
    ...
    if not connection_instance or getattr(connection_instance, "_sub2gen_send_patched", False):
        return
    if (
    ...
    connection_instance._sub2gen_send_patched = True
...
def _patch_nodriver_browser_instance(browser_instance):
    ...
    if getattr(browser_instance, "_sub2gen_update_targets_patched", False):
        return
    ...
    async def patched_update_targets(self, *args, **kwargs):
        try:
            ...
            raise
                ...
                if _is_runtime_disconnect_error(e):
                    try:
                        setattr(self, "_sub2gen_runtime_disconnected", True)
                    except Exception:
                        pass
                    try:
                        self.targets = []
        ...
        for target in list(getattr(self, "targets", []) or []):
            _patch_nodriver_connection_instance(target)
        try:
            setattr(self, "_sub2gen_runtime_disconnected", False)
        except Exception:
            pass
        return result

    browser_instance.update_targets = types.MethodType(patched_update_targets, browser_instance)
    browser_instance._sub2gen_update_targets_patched = True
...
class BrowserCaptchaService:
    ...
    def _is_browser_health_fresh(self) -> bool:
        if not (self._initialized and self.browser and self._last_health_probe_ok):
            return False
        try:
            if self.browser.stopped or getattr(self.browser, "_sub2gen_runtime_disconnected", False):
                return False
        except Exception:
            return False
    ...
    async def _probe_browser_runtime(self) -> bool:
        ...
        if not self.browser:
            ...
            return False
        if getattr(self.browser, "_sub2gen_runtime_disconnected", False):
            self._invalidate_browser_health()
            return False
    ...
    def _build_recaptcha_local_asset_hook_source(self, bundle: Dict[str, Any]) -> str:
        ...
        hook_source = f"""
            (() => {{
                ...
                window.__sub2genRecaptchaLocalAssets = bundle;
                if (window.__sub2genRecaptchaLocalAssetsPatched) {{
                    return Object.keys(fullMap).length;
                }}
                window.__sub2genRecaptchaLocalAssetsPatched = true;
        ...
        *,
        ...
        await self._tab_evaluate(tab, f"""
            (() => {{
                ...
                const stateKey = '__sub2genRecaptchaBootstrapState';
        ...
        async with self._browser_lock:
            ...
            if self._initialized and self.browser:
                try:
                    ...
                    elif getattr(self.browser, "_sub2gen_runtime_disconnected", False):
                ...
                if i in {4, 10, 18, 28}:
                    try:
                        last_bootstrap_state = await self._tab_evaluate(
                            ...
                            """
                                (() => {
                                    const state = window.__sub2genRecaptchaBootstrapState;
