...
- [ ] **Step 3:** `wrangler.jsonc` — worker name `drawstyle`; `d1_databases: [{binding: "DB", database_name: "drawstyle_db", database_id: "TBD-at-deploy", migrations_dir: "migrations"}]`; `r2_buckets: [{binding: "ASSETS", bucket_name: "drawstyle-assets"}]`; `vars: {OIDC_ISSUER: "https://account.leeguoo.com", OIDC_CLIENT_ID: "drawstyle-web", ADMIN_EMAILS: ""}`; `compatibility_date` = today. `vitest.config.ts` uses `defineWorkersConfig` with `wrangler: {configPath: "./wrangler.jsonc"}` and `miniflare: {d1Databases: ["DB"], r2Buckets: ["ASSETS"], bindings: {SESSION_SECRET: "test-secret", ADMIN_EMAILS: "admin@test.dev"}}`.
...
**Files:** create `migrations/0001_init.sql`, `src/db.ts`, `test/db.test.ts`.
